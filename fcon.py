...
def fcon(path):
    output('\n' + time.strftime('%X : ') +
           'Consolidating files in subfolders of ' + path + '\n')
    ...
    output('\n' + time.strftime('%X : ') + str(file_count) +
           ' files moved, in ' + str(round(time.time()-start_time, 3)) + ' seconds')
...
def consolidate_file(root, source_basename, destination_path, folder_count):
    ...
    destination_file_path = determine_unique_destination_file_path(
        source_basename, destination_path, prefix)
...
def determine_unique_destination_file_path(source_basename, destination_folder, prefix):
    destination_file_basename, destination_file_extension = os.path.splitext(
        source_basename)
    while (True):
        ...
        if (count_found):
            ...
            basename_without_count = re.search(
                r'^(.*)-[0-9]+$', destination_file_basename)
            destination_file_basename = basename_without_count.group(
                1) + '-' + str(count)
        else:
...
parser = argparse.ArgumentParser(
    description='Consolidate files in sub folders at path directly into folder at path')
parser.add_argument('--path', dest='path', required=False,
                    action='store', help='Target path')
parser.add_argument('--version', action='version', version=version)
parser.add_argument('--verbose', action='store_true', dest='verbose',
                    default=False, help='Will output extra info on logic')
